...
library(pacman)
p_load(tidyverse, ggplot2, terra, x3ptools, prettydoc, sf, raster,lsa )
...
img513 <- x3p_read("17BW-4XHA/scan-513.x3p")
img513 %>% 
  x3p_image()

## both are same objects. no issue about the importing method. 
...
img_list_use$`scan-006.x3p`$surface.matrix <- diff(img_list_use$`scan-006.x3p`$surface.matrix, lag=5)
...
#### This one is not necessary since we have the data-frame we wanted.

In the above image, we have centers in blue and radius is red. However, There are several spots and centers closed together, could be due to several lags detecting the same spot or duplicates. So, **Do the clustering again to remove the duplicates.**

Also, not that, here, we will use the original coordinate values and z values.

```{r}
## Final clustering based on center_x, center_y and intensity. 

## use the final_df_006 data frame
vector_coords <- final_df_006_updated[, c("x_center_original", "center_y", "radius","lyr.1")]
final_cluster <- dbscan(vector_coords, eps = 10, minPts = 2)  
final_cluster

# Add group label
final_df_006_updated$cluster_label <- final_cluster$cluster

## final dust spot summaries

unique_spots <- final_df_006_updated %>%
  dplyr::select(x_center_original, center_y, lyr.1, cluster_label, 
  lag, radius) %>% 
  rename("center_z" = lyr.1) %>% 
  filter(cluster_label != 0) %>% 
  group_by(cluster_label) %>%
  summarise(
    center_x = x_center_original[which.max(center_z)],
    center_y = center_y[which.max(center_z)],
    center_z = max(center_z),  ## keep peak intensity
    radius = max(radius), ## take the maximum radius
    lags_detected = paste(sort(unique(lag)), collapse = ", "),
    .groups = "drop"
  ) %>%
  mutate(dust_label = paste0("dust_", cluster_label)) %>% 
  as.data.frame()

```

```{r}
# reset original image raster
img_list_use <- img_list
r_006 <- rast(t(img_list_use$`scan-006.x3p`$surface.matrix))

# Plot image with centers and dust spots
plot(r_006, main = "Dust Spots Detected")
points(unique_spots$center_x, unique_spots$center_y, col = "blue", pch = 20, 
       cex = 1.2)
symbols(unique_spots$center_x, unique_spots$center_y, circles = unique_spots$radius, inches = FALSE, add = TRUE, fg = "red")
...
updated_coord_outliers_006_lag15 <- coord_outliers_006_lag15 + 15  # align with 

##obtain the corresponding original raster intensity values.
...
# Use either 2D or 3D coordinates
coords_used <- coords_3d[, 1:2]  # or scale(coords_3d) for x, y, z
...
results <- param_grid %>%
  rowwise() %>%
  mutate(
    model = list(dbscan(coords_used, eps = eps, minPts = minPts)),
    n_clusters = max(model$cluster),
    n_noise = sum(model$cluster == 0),
    silhouette = if (n_clusters > 1) {
      mean(silhouette(model$cluster, dist(coords_used))[, 3])
    } else {
      NA_real_
    }
  ) %>%
