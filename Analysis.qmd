...
for (img_name in names(img_list_use)[1:5]) {
  ...
  for (i in lags) {
    ...
    cluster_summary <- df_clustered %>%
      ...
      mutate(
        lag = i,
        image = img_name  ## Add image identifier
      )
...
final_df_updated <-  final_df %>% 
  group_by(lag, label, image) %>%
  mutate(dust_label = paste0(image, "_dust_", lag, "_", label)) %>% 
  mutate(original_center_x = center_x + lag)
...
final_df_all_img_updated <- final_df_updated %>%
  left_join(coord_val_df, 
            by = c("image", "original_center_x" = "x", "center_y" = "y"))

## only with the relevant info
final_df_all_img_updated %>% 
  dplyr::select(original_center_x, center_y, lyr.1, label, radius,
         lag,image,dust_label) %>% 
...
final_df_all_img_updated_006 <- final_df_all_img_updated %>% 
  filter(image == "scan-006.x3p")
...
final_df_all_img_updated_010 <- final_df_all_img_updated %>% 
  filter(image == "scan-010.x3p")
